---
title: Easy Setup
description: 'Track web analytics and custom events anywhere JavaScript is supported, all within seconds. We support Wordpress, Shopify and any other website builders.'
---

<img
  className="block dark:hidden"
  src="/images/techs.svg"
  alt="Hero Light"
/>
<img
  className="hidden dark:block"
  src="/images/techs.svg"
  alt="Hero Dark"
/>

## Pre-Requisites

Before proceeding, ensure you sign up on [Litlyx](https://dashboard.litlyx.com/) and create a new workspace.

## 1.a - Universal Setup (in just 30 seconds)

With a simple line of code in your **index.html**, inside the **body** or **head** tag, you can start tracking data such as `Best performing pages`, `Unique visitors`, `Real-time Users`, `Sources`, `Bouncing Rate`, `Countries` and much more.


```javascript 
<script defer data-workspace="workspace_id" 
src="https://cdn.jsdelivr.net/npm/litlyx-js/browser/litlyx.js">
</script>
```

## 1.b - Using any package manager

Install Litlyx library from your favourite package manager.

<CodeGroup>
```bash npm
npm install litlyx-js
```
```bash yarn
yarn add litlyx-js
```
```bash pnpm
pnpm add litlyx-js
```
```bash bun
bun install litlyx-js
```
</CodeGroup>

Now import litlyx in your `entry-point` file.

```javascript javascript
import { Lit } from "litlyx-js"
```


Next, initialize Litlyx library with your workspace in your entry-point file as follow:

```javascript javascript
Lit.init("workspace_id");
```

## Ready to go!

Explore [Litlyx's Dashboard](https://dashboard.litlyx.com/) with your web analytics collected in real-time.
<img
  className="block dark:hidden"
  src="/images/easy-dashboard.svg"
  alt="Hero Light"
/>
<img
  className="hidden dark:block"
  src="/images/easy-dashboard.svg"
  alt="Hero Dark"
/>
